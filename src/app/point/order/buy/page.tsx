...
const PointOrderBuy = () => {
  ...
  const cancelOrder = (id: string) => {
    setLoading(true);
    Api.Market.del_point_acquisition({ point_acquisition_id: id })
      .then(async () => {
  ...
  const loadOrders = () => {
    setLoading(true);
    Api.Member.get_point_acquisition()
      .then((res) => {
        set_acquisition_order(res.data);
      })
      .finally(() => {
  ...
  return (
    <div>
      <PointCardList
        items={acquisition_order}
        loading={loading}
        title="我的收購訂單"
        description="在這裡您可以查看和管理您發布的點數收購訂單。"
        emptyText="您還沒有發布任何點數收購訂單"
        renderItem={renderOrderCard}
        showPagination={acquisition_order?.length > pageSize}
        pageSize={pageSize}
